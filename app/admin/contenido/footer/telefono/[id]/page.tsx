  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setTelefono(data.descripcion || "");
      } catch (error: any) {
        setError(error.message || "Error al cargar el teléfono");
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="mx-auto max-w-2xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">
          {id === "nuevo" ? "Agregar" : "Editar"} Teléfono de Contacto
        </h1>
        <p className="mt-2 text-sm text-gray-600">
          Configura el teléfono que aparecerá en el footer
        </p>
      </div>
