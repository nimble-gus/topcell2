...
export default function EditarInstagramPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (!response.ok) {
          throw new Error("Instagram no encontrado");
        }

        const data = await response.json();
        setUrl(data.urlDestino || "");
      } catch (error: any) {
        setError(error.message || "Error al cargar Instagram");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || "Error al guardar Instagram");
      }
    ...
    <div className="mx-auto max-w-2xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">
          {id === "nuevo" ? "Agregar" : "Editar"} Instagram
        </h1>
        <p className="mt-2 text-sm text-gray-600">
          Configura la URL de Instagram que aparecerá en el footer
        </p>
      </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="https://www.instagram.com/topcell"
            />
          </div>
        </div>
